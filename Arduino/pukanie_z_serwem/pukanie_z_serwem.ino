...
void setup() {
 ...
 mojeserwo.attach(lockMotor); 
mojeserwo.write(servoClosed);
}
...
//digitalWrite(lockMotor, HIGH);
  ...
  if (!unlocked) {
  mojeserwo.write(servoOpen);
  digitalWrite(lockMotorPower, HIGH);
  
  delay (lockTurnTime);                    // Wait a bit.
  
  digitalWrite(lockMotorPower, LOW);            // Turn the motor off.
  }else{
      mojeserwo.write(servoClosed);
  digitalWrite(lockMotorPower, HIGH);
  ...
  digitalWrite(lockMotorPower, LOW); 
  }
  unlocked=!unlocked;
  delay(500);
